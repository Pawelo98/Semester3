...
CGeneticAlgorithm::CGeneticAlgorithm(int iPopSize, double dMutProb, double dCrossProb, double dGeneticTime)
{
	d_mutation_chance = dMutProb;
	d_crossover_chance = dCrossProb;
	i_population_size = iPopSize;
	d_genetic_time = dGeneticTime;
...
void CGeneticAlgorithm::vRun(CKnapsackProblem *cKnapsackProblem)
{
	...
	time_t t_finish = time(NULL);
	int i_iters = 0;

	while (t_finish-t_start<=d_genetic_time)
	{
		for (int g = 0; g < v_population.size(); g++)
		{
			if (i_iters == 0) v_population_fitnesses.push_back(v_population.at(g).dFitness(cKnapsackProblem));
			else if (i_iters == 1 && (v_population.size() % 2 == 1) && g == v_population.size() - 1) v_population_fitnesses.push_back(v_population.at(g).dFitness(cKnapsackProblem));
